...
#
# Architecture: browser (user N) → mitmdump:808N → Burp:8080 → target
#
# This proxy layer is BROWSER-AGNOSTIC — it serves BOTH backends:
#   - Chrome    (setup.sh)         → Chrome → mitmdump → Burp
#   - Lightpanda (setup-lightpanda.sh) → lightpanda mcp → mitmdump → Burp
# Run it once, regardless of which browser you configured.
#
# Usage:
